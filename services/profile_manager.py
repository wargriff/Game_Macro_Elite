...
class ProfileManager:
    ...
    def list_profiles(self) -> List[str]:
        if not os.path.isdir(self.profiles_dir):
            return ["default"]
        names = [f[:-5] for f in os.listdir(self.profiles_dir) if f.endswith(".json")]
    ...
    def load(self, name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not os.path.exists(path):
            log("PROFILE", f"no file at {path}, using defaults")
            self._data = self._default_data()
            return self._data
        with open(path, "r", encoding="utf-8") as f:
            self._data = json.load(f)
        log("PROFILE", f"loaded {self.current_name}", path=path)
        return self._data
    ...
    def save(self, name: Optional[str] = None) -> None:
        ...
        log("PROFILE", f"saved {self.current_name}")
    ...
    def apply_to_engine(self, manager: MacroManager) -> None:
        ...
        log("PROFILE", "applied to engine", keys=list(macro.keys()))
    ...
    def _default_data(self) -> Dict[str, Any]:
        return {"rgb": {}, "bindings": {}, "macro": {"buttons": {}}}
